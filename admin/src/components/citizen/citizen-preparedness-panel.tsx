...
import { PREPARE_GUIDES, type PrepareTopicId } from "@/lib/preparedness-guides";
...
export function CitizenPreparednessPanel(props: {
  ...
  const guideById = new Map(PREPARE_GUIDES.map((g) => [g.id, g]));
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[11px] text-zinc-500 leading-relaxed">
        Tap a card for step-by-step guidance. Use the checkmark when you have completed each item.
      </p>

      <ul className="grid gap-3 sm:grid-cols-2">
        {props.checklist.map((item) => {
          const meta = guideById.get(item.id as PrepareTopicId);
