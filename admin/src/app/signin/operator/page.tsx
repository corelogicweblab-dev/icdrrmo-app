...
export default function SigninOperatorPage(): ReactElement {
  ...
  async function onSubmit(e: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      if (!p?.role || !ALLOW.has(p.role)) {
        clearOpsTokens();
        setMsg("This portal is for operator / desk roles only (ADMIN, SUPER_ADMIN, OPERATOR).");
        return;
      }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-transparent text-zinc-100 px-4">
      <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-gradient-to-b from-rose-950/30 to-black/60 p-8 shadow-panel">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-lg font-semibold tracking-tight">Operator / desk sign-in</h1>
            <p className="text-[11px] text-zinc-500">Command console after JWT sign-in.</p>
          </div>
        </div>
