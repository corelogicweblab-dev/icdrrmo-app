...
export default function ResponderProfilePage(): ReactElement {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto space-y-6">
      ...
      {saved ? <p className="text-xs text-orange-400">Medical details saved.</p> : null}
      <BarangayUserProfileCard accessToken={access} requireBarangay />
      {!me ? (
        <Loader2 className="h-6 w-6 animate-spin text-zinc-500" aria-hidden />
      ) : (
        <form onSubmit={(ev) => void onSaveMedical(ev)} className="space-y-6">
          <OpsPanelCard title="Field medical & availability">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="block space-y-1 text-xs sm:col-span-2">
                <span className="text-zinc-500">Medical conditions</span>
                <textarea
              ...
              <label className="block space-y-1 text-xs sm:col-span-2">
                <span className="text-zinc-500">Emergency notes</span>
                <textarea
